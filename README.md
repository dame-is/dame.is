...
- `is.dame.state` / `is.dame.state.sample` — live iPhone-sourced physical +
  ambient state (heart rate, activity, battery, ambient sound, calories). The
  singleton at `at://{me}/is.dame.state/self` is the current "right now",
