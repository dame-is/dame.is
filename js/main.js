...
for (const [headerDateText, groupData] of Object.entries(groupedPosts)) {
    ...
    groupData.posts.forEach(item => {
        ...
        if (post && post.record) {
            ...
            const localPostLink = document.createElement('a');
            localPostLink.href = createPostLink(post);
            localPostLink.textContent = 'View post';
            localPostLink.classList.add('local-post-link');
            postDateElem.appendChild(document.createTextNode(' • '));
            postDateElem.appendChild(localPostLink);
...
async function loadSinglePost(postId) {
    ...
    try {
        ...
        const apiUrl = `https://enoki.us-east.host.bsky.network/xrpc/com.atproto.repo.getRecord?repo=did:plc:gq4fo3u6tqzzdkjlwzpb23tj&collection=app.bsky.feed.post&rkey=${postId}`;
        ...
        // Create a post object in the format expected by our existing rendering code
        const post = {
            uri: data.uri,
            cid: data.cid,
            record: data.value,
            ...
            repostCount: 0,
            likeCount: 0,
            indexedAt: data.value.createdAt
        };
        ...
        if (post.record.embed) {
            // Reuse the embed handling code from the post loader
            if (post.record.embed.$type === "app.bsky.embed.recordWithMedia#view") {
                // Handle record with media
                if (post.record.embed.media && post.record.embed.media.$type === "app.bsky.embed.images#view" && Array.isArray(post.record.embed.media.images)) {
                    post.record.embed.media.images.forEach(imageData => {
                        if (imageData.fullsize) {
                            const img = document.createElement('img');
                            img.src = imageData.fullsize;
                            img.alt = imageData.alt || 'Image';
            ...
            } else if (post.record.embed.$type === "app.bsky.embed.images#view" && Array.isArray(post.record.embed.images)) {
                // Handle images
                post.record.embed.images.forEach(imageData => {
                    if (imageData.fullsize) {
                        const img = document.createElement('img');
                        img.src = imageData.fullsize;
                        img.alt = imageData.alt || 'Image';
        ...
        // Clear loading indicator and append the post
        if (loadingIndicator) {
            loadingIndicator.style.display = 'none';
        }
        container.innerHTML = '';
