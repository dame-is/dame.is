...
import { brokenTakes, takesPresent } from './ratioedParticipant.js';
import { composeEventLog, livingKindsIndex } from './ratioedLog.js';
...
// Both live in ratioedParticipant.js, which is import-graph-clean for the Open
// Graph renderer; brokenTakes is re-exported here because this is where every
// caller has always found it.
export { brokenTakes };
export { composeEventLog };
...
export function livingRoster(pieces, people = SEED_PEOPLE, events = null) {
  const liveKinds = livingKindsIndex(events, people);
  ...
  // Breaking a piece IS being there, whatever survives of it — see
  // takesPresent, which is the rule and is shared with the OG renderer.
